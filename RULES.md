...
- **A. 质量闸门（second-pass verifier）**：总结后再调一次 AI 按 7 评分维度打 0–100 分（含篇幅达标），低于阈值带反馈重试一次。**默认关闭**（省一轮 AI 调用）；开关见下方「去哪里开关」。无外部 AI 的降级路径走 `QUALITY_GATE_SELFCHECK` 自检段（模板 prompt 内嵌三条专项自检——思维模型深挖 + 固定结构合规 + 篇幅区间，与 `UNIVERSAL_RULES` §八红线不重复，外层模型自核对）。**语义层质检（通顺/完整/忠实）的实际落地**：机械门禁读不懂语义，故由**父 Agent 按 `prompts/review_rubric.py` 的五维 rubric（合成度/完整性/通顺度/忠实度/合规度）抽检产出**，命中问题就地修（fix_inline）或打回重做（resummarize）；无父 LLM 在环时降级为入 `notes/_review_queue.jsonl` 等人工/外接AI审。
- **A2. 机械落盘门禁（零 AI 依赖 · 2026-09-05，去 H1 更新 2026-09-07，字数改 source-aware · 2026-09-13）**：`prompts/verifier.py` 的 `verify_note_mechanical` 在 `save_summary_only` 落盘前机械校验：**正文 0 个一级标题**（去 H1：标题由文件名/飞书节点标题承担，扫描前剥离代码围栏，`#` 注释不误伤）；正文不写来源链接行、不出现原文 URL（防 formatter 权威追加后落盘出现两个链接）；**字数改为「源长×比例带的参考值」**（有源长时按 `源长×(0.25,0.45)` 算参考带，仅超/偏出带置 `review_flags` 触发父 Agent 抽检，不硬拦；仅两种极端硬拦——`>源长×0.7`（畸高照搬/未合成）与 `<400`（内容缺失）；无源长退回 `NOTE_WORD_LIMITS` 固定区间兜底）。另含 `detect_compression_issues` 过度压缩启发式（超长句/连环括号/逗号密度），命中只报 warning 不拦。接入顺序：dedup 闸门之后（已总结出队优先）、folder 自动路由之前；卫生类（H1/来源链接）问题**永不放行**；字数卫生外问题拦截返回 `VERIFIER_FAILED:<issues>`，不落盘不 dedup，子 Agent 按 issues 修复重试；`force` 只豁免 dedup 不豁免本门禁。`review_flags` 非空时 `save_summary_only` 自动写 `notes/_review_queue.jsonl`（无人值守降级：落盘+标 needs_review 等人工/外接AI审，绝不假定语义 OK）。详见 `docs/decisions/DECISION-20260913-gate-source-aware-review.md`。
...
- [ ] **机械落盘门禁（默认生效 · 2026-09-05，source-aware 参考值 · 2026-09-13）**：`save_summary_only` 落盘前自动过 `verify_note_mechanical`（去 H1 / 来源链接卫生 / **源长×比例带参考值** + 过度压缩启发式）；卫生类（H1/来源链接）**永不放行**；字数仅两种极端硬拦（畸高照搬 `>源长×0.7` / 内容缺失 `<400`），其余越界/超参考带只置 `review_flags` 触发父 Agent 抽检、不阻塞落盘；被拦返回 `VERIFIER_FAILED:<issues>` 时子 Agent 应按 issues 修复后重试同一入口；`review_flags` 非空自动入 `notes/_review_queue.jsonl`（无人值守降级）。语义层（通顺/完整/忠实）只能由父 Agent 或外部 AI 判，机械层不越权。详见 `docs/decisions/DECISION-20260913-gate-source-aware-review.md` 与 `prompts/review_rubric.py`。
