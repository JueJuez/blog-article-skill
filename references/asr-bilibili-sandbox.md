...
- **实现位置**：`videos/fetch.py::bili_is_charging_exclusive()`（判据，**仅用于打印原因，不再作拦截依据**）
  + `LAST_CHARGING_EXCLUSIVE`（模块级标记）；`videos/asr.py::transcribe_video` 的 90% 覆盖率闸门
  （**唯一硬闸门**）。⚠️ 监控发现路径（`is_charging_arc`）一直有过滤；**漏的是补源/补齐路径**
  （走 `ugc_season` 拿不到该字段）。
- **两条明确「不做」的决定**（别再自己长回来）：① 字幕侧不加「源字/秒」密度校验——用户反馈没遇到过
  字幕不完整，别为没见过的场景加机制；② 不设「已知充电+无字幕就直接记不可达」的快速路径——
  会误杀「短视频 + 试看恰好完整」这类**真能收录**的集，正是这次修掉的错误。
  代价是这类集要先下载一次试看片段（实测最大 ~43MB）才被闸门拒，**用户已确认接受**。
- **阈值为什么定 90%**：截断发生在**前中段**（试看片段就是典型形态），不会刚好差一点到 90%，
  所以「≥90% 就放行」的残留风险不符合实际（用户 2026-09-20 明确判断）。
