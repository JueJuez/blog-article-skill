...
- **系列课（series）**：某个 B站UP主发布的成套视频（如课程、专栏）。监控时按 `monitors/subscriptions.json` 的 `series_patterns` 识别；单集与散文一样逐 URL 走五步管线（PLAN-20260908），防重靠登记表 URL 键，不主动回溯旧集、点名才补（`scripts/backfill_series.py`）。
- **系列容器（series container）**：飞书知识库里承载某个系列课所有集笔记的节点，挂在 `【监控】/<平台>/<UP>/<系列名>/` 下（不是根）。由 `shared/routing.py:resolve_folder` 统一算路径；`_save_series_note` 收到的 `folder` 只到账号层，系列容器由 `ensure_series_node` 单建，避免嵌套。
...
- **两 pending 队列**：
  - `monitors/pending_summaries.json`：待总结的单篇（文章/视频；系列课单集同队列）。
