...
- **运行**
  ...
  - **新会话执行步骤（照做即一帆风顺）**：
    ...
    4. 运行结束后，本会话（执行模型）**必须**在本次会议内闭环两类待总结队列（全自动，无需用户手动命令）：
       ...
       - ⚠️ **落盘门禁 retry 契约（2026-09-18 统一）**：`save_summary_only` 返回 `VERIFIER_FAILED`（机械：H1/来源链接/URL/字数）或 `VERIFIER_CONTENT_FAILED`（内容：结构缺失/锚点召回不足/照搬未合成，带 `retry:true`）时，子 Agent 按 `issues` 重写后重交（≤2 次）；仍失败则 `--force` 逃生或跳过。**队列路径此前只有机械门禁、内容判据是软抽检，现与重抓 `resum_save_batch` / 系列课 `_save_series_note` 对齐为「内容缺陷也拦 + 重试」**——这正是补齐的缺口。
