...
- **消费队列**：一律走 `scripts/consume_migrate_queue.py`（2026-09-09 落地，2026-09-10 加无CC暂缓；测试 `tests/test_consume_migrate_queue.py` 43 例）——`--plan`（registered/deferred/to_fetch 进度）→ `--fetch --limit 15`（抓正文入 staging `notes/_scraped/migrate/pending_summaries.json`，prompt/note_type 预计算，B站标题 difflib 相似度 <0.55 或失败 3 次转 manual，412 熔断；**无 CC 字幕条目直接暂缓**——探测 `fetch_transcript` 返回 None 即标 `deferred`，不重试、不占 limit 配额）→ 执行模型派子 Agent 按 staging 内 prompt 总结并 `articles/_save_summary.py` 落盘（自动登记）→ `--clean`（已登记出队 done）→ 循环至清零 → `--cleanup-old --apply`（先 dry-run：登记命中才删旧文）。**无 CC 暂缓维护**：`--defer-failed` 把历史无CC失败批量暂缓（幂等）；`--classify-deferred`（默认 dry-run，`--apply` 落盘）三分类——**view API 先行**（2026-09-10 v2 修熔断盲点：`fetch_transcript` 吞异常返 None，412 到不了外层熔断）：code `-404`/`62002`→`removed_video` 定档（跳过字幕探测）；连续 2 次请求层异常=风控熔断 abort；环境健康才探测字幕——有字幕=误标→清 `deferred`+`fails` 清零回队，无字幕→`no_cc_confirmed`。路由：`bili_video`→`summarize_video`、`article`→`skill_main`（folder=folder_hint 落同目录）、`manual_no_url`→人工补链接。脚本不加载 .env，运行须显式 `--archive-dir "<vault同级>/_migrate_gate_archive"`。
