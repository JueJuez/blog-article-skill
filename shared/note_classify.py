...
_PARENT_PATTERNS = [
    ...
    # 情感/两性域（2026-09-16 新增）：解决恋爱内容被词频误判进 投资/商业/个人成长 的问题——
    # 旧词表完全没有两性词，恋爱干货里偶现的「职场/认知/搞钱」词就抢走父域（212 批实测误判 22 篇）。
    # 用高精度词（恋爱/追女生/相亲…），不用裸「女生」「情感」「亲密关系」——这三个是心理/成长内容常用词，
    # 加入后会把心理学综合稿误判成情感域（test_inbox_fallback_no_domain_when_comprehensive 即此案例）。
    ("情感/两性", r"恋爱|追女生|追女孩|追男生|脱单|相亲|约会|分手|挽回|表白|暧昧|择偶|两性|婚恋|女朋友|男朋友|情感咨询|恋爱干货"),
]
...
def subdomain_from_lede(parent, lede):
    ...
    if parent == "个人成长":
        ...
        return "个人成长·综合"
    if parent == "情感/两性":
        if re.search(r"追女生|追女孩|追男生|搭讪|吸引|暧昧|表白|心动", s):
            return "追求与吸引"
        if re.search(r"约会|聊天|话题|微信|回复|朋友圈", s):
            return "约会与聊天"
        if re.search(r"恋爱|相处|吵架|安全感|仪式感|异地|价值观", s):
            return "恋爱相处"
        if re.search(r"分手|挽回|复合|失恋|断联", s):
            return "分手与挽回"
        if re.search(r"相亲|择偶|结婚|婚恋|彩礼|见家长", s):
            return "相亲与婚恋"
        return "情感·综合"
    return "综合"
