...
> ⚠️ **已取代（2026-08-08）**：本决策写于"Obsidian + 飞书强制双写"时代。输出规则已被
> `DECISION-20260808-obsidian-optin.md` 改为 **默认只写飞书、Obsidian 按需**。
> 下文"双写"表述仅作历史记录，不再代表当前默认行为；当前默认请以后者为准。
...
- 认证：B站用根目录 `.env` 的 `BILI_COOKIE`（登录态 Cookie，动态接口硬性要求）；公众号用 `monitors/.wechat_auth.json` 里的 JWT（转发服务器签发，数小时失效，`run.py` 检测到失效则本次跳过公众号、保 B站，交互式会话弹码续期、下次运行恢复）。
