# ⚠️ 部分已被取代（2026-09-15 · content-first 门禁）
>
> 本文建议 5 的「自检三条（+篇幅区间）/ 评分 7 维（+篇幅达标）」已升级为：自检**四条**（+覆盖自检、篇幅自检）、
> 评分第 7 项改为「**内容完整（权重最高）**」；字数改为两档硬拦（`<300` / `>max(8000,源长×3)`），
> 比例带废除。现行规则见 `docs/decisions/DECISION-20260915-content-first-gate.md`。其余内容仍有效。
