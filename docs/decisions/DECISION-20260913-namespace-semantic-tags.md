...
## 六、补丁（2026-09-13 下午）：标签去重 + 分类器纠错

上线后消费两篇夏鹏（读书/职场 UP）笔记，发现标签行存在**重复**与**不适配**两类问题，已修复：

- **重复**：入队预填的裸作者标签（如 `夏鹏本鹏`）与方案 A 命名空间标签 `来源/夏鹏本鹏` 同义双写。修复：`save_summarized_article` 注入语义标签后，移除被命名空间覆盖的裸标签。
- **不适配（错归投资）**：`note_classify.py` 把 `【我的总结】/作者/<账号>` 一律无脑兜底 `投资`，且 lede 二次校验刻意排除 `parts[1]=="作者"`，导致夏鹏被归 `#投资/系统`、`#投资/综合`。修复：①作者归档笔记 `parent=None` 交导语粗判；②开放 lede 二次校验给作者归档笔记；③新增 `个人成长` 父域（习惯/职场/读书/成长/认知等关键词）→ 夏鹏正确归 `#个人成长/习惯效率`、`#个人成长/职场`。
- **存量不一致（缺基础标签）**：传了 tags 时不再补 `#文章总结`，与存量 1087 篇不一致。修复：`save_summarized_article` 收口时强制确保 `#文章总结`（或 `#原创`/`#动态`）存在。
- **验证**：`tests/test_note_classify.py` 新增 夏鹏用例（author-archived 不得归投资 + 裸作者被移除 + 基础标签补齐），13 项全过；回归 76 项通过。两篇夏鹏笔记已用修正逻辑 `force` 重落真实库。
