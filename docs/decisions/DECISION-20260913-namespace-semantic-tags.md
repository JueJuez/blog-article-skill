...
- **遗留（待决/观察）**：
  1. 离线索引原型 `_tmp/classify_v4d.py` 仍是 `shared/note_classify` 的独立副本；将来重跑 `index` 时应让它 `import shared.note_classify` 保持完全同源（当前未动，避免改动既有 index 快照）。
  2. 第 2 项决策（自动 vs 手填）按**自动生成**落地；若后续想要"AI 总结时顺手抽、不强制自动"的混合模式，改 `save_summarized_article` 注入逻辑即可。
