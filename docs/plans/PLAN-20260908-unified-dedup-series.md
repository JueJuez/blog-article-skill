...
> 背景：全管线去重审计（原 `audit_pipeline_dedup_20260906.md`，已归档至 `_archive/`）发现判定分散在 5+ 处状态文件；migrate_gate v3 DRY-RUN 扫出 652 文件中 457 异常、重抓队列 426 条；系列课六件套（贪婪抓取/双索引/队列/manifest/页码防护）被实证为过度设计。用户三项决策已拍板（见 §10）。
