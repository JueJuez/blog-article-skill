...
**条件一：用户说了这些词**（任一即可）
- 总结 / 提炼 / 整理 / 归档 / 保存笔记
- 补齐scys / 补齐生财有术（**scys 批量补齐触发词**：自动跑 `scripts/scys_batch_fetch.py` 四领域默认参数，后缀自然语言改领域/时间/含非精华——语义表见 `references/scys-fetch-sop.md` §9，无需再问用户流程。**此触发词单独成立，不需要满足条件二**）
...
总结后有一道「第二遍 AI 把关」（质量闸门）：按 6 红线打 0–100 分，低于阈值带反馈重试一次。**默认关闭**（省一轮 AI 调用）。

- **去哪里开关**：在技能根目录的 `.env` 设置：
  ```env
  NOTE_QUALITY_GATE=1       # 默认 0（关）；置 1 开启
  NOTE_GATE_THRESHOLD=85    # 评分阈值，默认 85
  ```
- **降级路径（无外部 AI Provider）**：闸门无法调 AI，会自动走「自检闸门」——模板 prompt 内嵌 6 红线，由你（外层对话模型）写完后自行核对，无需手动开启。
- 详见 `references/config.md` §九 与 `RULES.md` §4.6。
